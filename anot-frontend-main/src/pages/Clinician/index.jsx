...
/**
 * Build calendar grid cells for a month (null = padding, number = day of month).
 */
...
/**
 * Format month label for calendar header.
 */
function formatCalendarMonthLabel(viewYear, viewMonth) {
  return new Date(viewYear, viewMonth, 1).toLocaleDateString('en-US', { month: 'long', year: 'numeric' })
}

/**
 * Shift calendar view by one month.
 */
function shiftCalendarMonth(viewYear, viewMonth, delta) {
  const d = new Date(viewYear, viewMonth + delta, 1)
  return { year: d.getFullYear(), month: d.getMonth() }
}

/**
 * Close calendar popup when clicking outside anchor/popup.
 */
function useCalendarOutsideDismiss(popupRef, anchorRef, onClose) {
  ...
  }, [onClose, anchorRef, popupRef])
}

function CalendarPopupHeader({ monthLabel, onPrev, onNext }) {
  return (
    <div className="cl-calendar-popup__head">
      <button type="button" className="cl-calendar-popup__nav" onClick={onPrev} aria-label="Previous month">
        ‹
      </button>
      <span className="cl-calendar-popup__month">{monthLabel}</span>
      <button type="button" className="cl-calendar-popup__nav" onClick={onNext} aria-label="Next month">
        ›
      </button>
    </div>
  )
...
function CalendarDayCell({ day, index, viewYear, viewMonth, selectedOff, onSelectDate }) {
  if (day === null) {
    return <span key={`empty-${index}`} className="cl-calendar-popup__cell cl-calendar-popup__cell--empty" aria-hidden />
  }
  const dayOff = daysOffsetFromToday(viewYear, viewMonth, day)
  const isToday = dayOff === 0
  const isSelected = dayOff === selectedOff
  return (
    <button
      key={day}
      type="button"
...
/**
 * Schedule Date Picker — calendar popup for choosing a schedule day offset.
 */
function ScheduleDatePicker({ off, onSelectDate, onClose, anchorRef }) {
  ...
  useCalendarOutsideDismiss(popupRef, anchorRef, onClose)

  const cells = buildCalendarCells(viewYear, viewMonth)
  const monthLabel = formatCalendarMonthLabel(viewYear, viewMonth)
  ...
  return (
    <div className="cl-calendar-popup" ref={popupRef} role="dialog" aria-label="Choose date">
      <CalendarPopupHeader monthLabel={monthLabel} onPrev={prevMonth} onNext={nextMonth} />
...
function AIModalHeader({ visit, hideAudioControls, txBadge, txSt, txBusy, loading, onRunTx, onClose }) {
  return (
...
function AIModalDraftContent({ secs }) {
  if (!secs) {
    return (
  ...
  return NOTE_DRAFT_SECTIONS.map(({ k, l, icon }) => (
...
function AIModalTranscriptContent({ txts, recIdx, onRecIdxChange }) {
  if (txts.length === 0) {
    return <div style={{ textAlign:'center', padding:60, color:'#94A3B8' }}>Your scribe team is preparing the transcript.</div>
  }
  return (
    <>
      {txts.length > 1 && (
...
/**
 * AI Modal — note preview with scribe draft and transcript tabs.
 */
function AIModal({ visit, onClose, showToast, hideAudioControls = false }) {
  ...
  const txSt = note?.transcription_status || visit?.transcription_status
  const txBadge = getTranscriptionBadgeText(txSt)
  const txts = parseNoteTranscriptions(note)
  const secs = parseNoteDraftSections(note?.ai_draft)
  ...
  return (
    ...
    >
      <div
        ...
        aria-modal="true"
      >
        <AIModalHeader visit={visit} hideAudioControls={hideAudioControls} txBadge={txBadge} txSt={txSt} txBusy={txBusy} loading={loading} onRunTx={runTx} onClose={onClose} />
        <AIModalTabBar tab={tab} onTabChange={setTab} hideAudioControls={hideAudioControls} />
        <div style={{ flex:1, overflowY:'auto', padding:'20px 24px' }}>
          {loading ? (
            <div style={{ textAlign:'center', padding:60, color:'#94A3B8' }}>Loading note...</div>
          ) : tab === 'ai' || hideAudioControls ? (
            <AIModalDraftContent secs={secs} />
          ) : (
            <AIModalTranscriptContent txts={txts} recIdx={recIdx} onRecIdxChange={setRecIdx} />
          )}
        </div>
      </div>
    </div>
...
// Mirrors the backend's extractSectionHeaders (utils/noteTemplateSections.js): a header
// line either ends with ':' or is immediately followed by a Setext-style underline row.
function countTemplateSections(content) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.length <= 80 && /^[-=_—–]{3,}$/.test(lines[i + 1] || '')) {
      count++
    }
  }
  return count
...
function updateTemplateInList(templates, templateId, content) {
  return templates.map((t) => (t.id === templateId ? { ...t, content } : t))
}
...
const CLINICIAN_NAV_ITEMS = [
  { key:'schedule', label:'Schedule', icon:'📅', badgeKey: 'schedule' },
  { key:'notes',    label:'Notes',    icon:'📝', badgeKey: 'urgent' },
  { key:'contact',  label:'Contact Us', icon:'💬' },
]

function ClinicianNavItem({ item, screen, badge, onNavigate }) {
  return (
    <div
      role="button"
      tabIndex={0}
      className={`sf-nav-item sf-sidebar-rich__nav-item${screen === item.key ? ' active' : ''}`}
    ...
    >
      ...
      {badge > 0 ? (
        <span className={`sf-sidebar-rich__nav-badge sf-sidebar-rich__nav-badge--${item.badgeKey}`}>
          {badge}
        </span>
      ) : null}
    </div>
...
function ClinicianSidebarNav({ screen, scheduleUpcomingBadge, readyForReviewCount, onNavigate }) {
  const badges = { schedule: scheduleUpcomingBadge, urgent: readyForReviewCount }
  return (
    ...
    <>
      ...
      >
        ...
        <div className="cl-sidebar__body">
          ...
          <ClinicianSidebarNav
            screen={screen}
            scheduleUpcomingBadge={scheduleUpcomingBadge}
...
function StandardClinicianPortal() {
  ...
  /**
   * Reset recording state after visit ends or upload queues
   */
  ...
  /**
   * Update visit list after successful end
   */
  const applyEndedVisitToState = (visitId, endData, patientName, duration) => {
  ...
  /**
   * Cancel/discard active recording and restore visit to upcoming
   */
  const cancelRecording = async () => {
    ...
    try {
      ...
      showToast(err.message || 'Failed to cancel recording', 'error')
      resetRecordingState()
    }
  }

  /**
   * Resume recording for an orphaned/in-progress visit
   */
  const resumeOrphanedVisit = (v) => {
    if (active || addRec) {
      showToast('Another recording is already active.', 'warn')
      return
    }
    startVisit(v)
  }
  ...
  /**
   * End an orphaned in-progress visit directly
   */
  const endOrphanedVisit = async (v) => {
    try {
      setUploading(true)
      const endData = await visitsAPI.endVisit(v.id, v.duration_seconds || 0)
      applyEndedVisitToState(v.id, endData, v.patient_name, v.duration_seconds || 0)
    } catch (err) {
      try {
        await visitsAPI.updateStatus(v.id, 'recording-uploaded')
        setVisits(p => p.map(x => x.id === v.id ? { ...x, status: 'recording-uploaded' } : x))
        showToast(`✓ Visit marked as With Scribe for ${v.patient_name}`)
      } catch {
  ...
  /**
   * Reset an in-progress or interrupted visit back to upcoming
   */
  const resetVisitToUpcoming = async (v) => {
    try {
      await visitsAPI.updateStatus(v.id, 'upcoming')
      setVisits(p => p.map(x => x.id === v.id ? { ...x, status: 'upcoming' } : x))
      showToast(`Visit reset to upcoming for ${v.patient_name}`)
    } catch (err) {
      showToast(err.message || 'Failed to reset visit', 'error')
    }
  }
  ...
  /**
   * End Visit Handler — orchestrates audio processing, visit completion, state reset
   */
  ...
  const openNoteDetail = async (visitRow) => {
    try {
      ...
      setReview({
        ...
        ai_draft: note?.ai_draft ?? visitRow.ai_draft,
  ...
  const startEditingNote = () => {
    setEditedNoteContent(reviewNote.final_note || cleanAiDraftForDisplay(reviewNote.ai_draft) || '')
    setEditingNote(true)
  }
  ...
  // Live updates: while the review screen is open and the note isn't locked
  // yet, silently refetch so scribe edits/AI draft changes show up without a
  // manual refresh. Paused while the doctor is mid-edit to avoid clobbering
  // their in-progress text.
  useEffect(() => {
    if (!reviewNote?.id || !!reviewNote?.locked_at || editingNote) {return undefined}
    const timer = setInterval(async () => {
      try {
        const note = await getTranscription(reviewNote.id)
        if (!note) {return}
        setReview((prev) => (prev ? {
          ...prev,
          final_note: note.final_note ?? prev.final_note,
          ai_draft: note.ai_draft ?? prev.ai_draft,
          note_status: note.status ?? prev.note_status,
          locked_at: note.locked_at ?? prev.locked_at,
        } : prev))
      } catch {
  ...
  }, [reviewNote?.id, reviewNote?.locked_at, editingNote])
              ...
              {active && (
                <div className="sf-rec-banner" style={{ marginBottom: 14 }}>
                  ...
                  <div className="sf-audio-controls">
                    ...
                    <button type="button" className="btn btn-sm sf-btn-ghost" onClick={cancelRecording} style={{ background: '#F1F5F9', color: '#475569', border: '1px solid #CBD5E1' }}>Cancel</button>
                  </div>
                </div>
              )}
                  ...
                  return (
                    ...
                    <div className={`cl-schedule-row sf-row cl-patient-card ${accentClass} ${canonicalCardClass}${scribeUrgentOnSchedule ? ' cl-patient-card--scribe-urgent' : ''}${isActive ? ' sf-row-active' : ''}`}>
                      ...
                      <div className="cl-schedule-row__actions">
                        ...
                        {v.status === 'in-progress' && isActive && (
                          <div style={{ display:'flex', gap:8, flexWrap:'wrap', justifyContent:'flex-end', alignItems:'center' }}>
                            <div style={{ display:'inline-flex', alignItems:'center', gap:6, padding:'6px 14px', borderRadius:10, background: paused ? '#FEF3C7' : '#FFF1F2', border: paused ? '1px solid #FDE68A' : '1px solid #FECDD3' }}>
                              <span style={{ width:8, height:8, borderRadius:'50%', background: paused ? '#D97706' : '#E11D48', display:'inline-block' }} />
                              <span style={{ fontSize:13, color: paused ? '#92400E' : '#9F1239', fontWeight:700 }}>
                                {paused ? 'Paused' : 'Live'} {fmtSecs(timer)}
                              </span>
                            </div>
                            <button type="button" className="btn btn-sm btn-amber" onClick={pauseResume} style={{ borderRadius:10, fontWeight:700 }}>
                              {paused ? '▶ Resume' : '⏸ Pause'}
                            </button>
                            <button type="button" className="btn btn-sm btn-red" onClick={endVisit} style={{ borderRadius:10, fontWeight:700 }}>
                              ■ End
                            </button>
                            <button type="button" className="btn btn-sm" onClick={cancelRecording} style={{ borderRadius:10, background:'#F1F5F9', color:'#475569', border:'1px solid #CBD5E1', fontWeight:600 }}>
                              Cancel
                            </button>
                          </div>
                        )}
                        {v.status === 'in-progress' && !isActive && (
                          <div style={{ display:'flex', gap:8, flexWrap:'wrap', justifyContent:'flex-end', alignItems:'center' }}>
                            <ClinicianTooltip tip="Resume or re-record audio for this encounter">
                              <button type="button" className="cl-schedule-cta" onClick={() => resumeOrphanedVisit(v)} disabled={!!active || !!addRec} style={{ display:'inline-flex', alignItems:'center', gap:6, padding:'8px 18px', borderRadius:12, background:(active||addRec)?'#94A3B8':'linear-gradient(135deg,#16A34A,#15803D)', color:'#fff', border:'none', fontSize:14, fontWeight:800, cursor:(active||addRec)?'not-allowed':'pointer' }}>
                                <IconMic />
                                Resume Recording
                              </button>
                            </ClinicianTooltip>
                            <ClinicianTooltip tip="End encounter and send to scribe">
                              <button type="button" className="btn btn-sm btn-navy" onClick={() => endOrphanedVisit(v)} style={{ borderRadius:10, padding:'8px 16px', fontWeight:700 }}>
                                End Visit
                              </button>
                            </ClinicianTooltip>
                            <ClinicianTooltip tip="Reset encounter back to upcoming status">
                              <button type="button" className="btn btn-sm" onClick={() => resetVisitToUpcoming(v)} style={{ borderRadius:10, padding:'8px 12px', background:'#F1F5F9', color:'#64748B', border:'1px solid #E2E8F0', fontWeight:600 }}>
                                Reset
                              </button>
                            </ClinicianTooltip>
                          </div>
                        ...
                        {/* recording-uploaded: NO "Generate Note" button.
                            ...
                            Preview Draft (opens the review screen early) + Add to Encounter for additional audio. */}
                        {v.status === 'recording-uploaded' && (
                          <div style={{ display:'flex', gap:8, flexWrap:'wrap', justifyContent:'flex-end', alignItems:'center' }}>
                            <ClinicianTooltip tip="View the AI-generated draft while the scribe is still working on it">
