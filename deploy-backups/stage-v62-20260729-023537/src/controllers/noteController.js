...
async function ensureEhrColumns() {
  ...
  await addColumnIfMissing('notes', 'ehr_encounter_id', 'ALTER TABLE notes ADD COLUMN IF NOT EXISTS ehr_encounter_id VARCHAR(64)')
  await addColumnIfMissing('notes', 'ehr_connection_id', 'ALTER TABLE notes ADD COLUMN IF NOT EXISTS ehr_connection_id INTEGER REFERENCES ehr_connections(id) ON DELETE SET NULL')
}

/**
 * Pushes the finalized note into whichever EHR the visit's clinician is
 * configured for (users.ehr_connection_id), lazily creating the patient's
 * external record on first push. Returns { skipped: true } when the
 * clinician has no EHR connection assigned, or the connection isn't
 * enabled/fully configured (current no-op behavior preserved). Throws on a
 * genuine EHR API failure so the caller can surface a 502 instead of
 * silently marking the note as uploaded.
 */
async function pushNoteToEhrIfConfigured({ visit, finalNote, client }) {
  const clinicianRow = await client.query('SELECT ehr_connection_id, ehr_provider_id FROM users WHERE id = $1', [visit.clinician_id])
  const clinician = clinicianRow.rows[0]
  if (!clinician?.ehr_connection_id) return { skipped: true }

  const connection = await ehrConnectionsService.getConnection(clinician.ehr_connection_id)
  if (!connection || !connection.enabled) return { skipped: true }
  if (!ehrConnectionsService.hasCompleteCredentials(connection.ehr_type, connection.credentials)) return { skipped: true }

  const driver = getDriver(connection.ehr_type)

  const patientRow = await client.query(
    'SELECT id, name, mrn, date_of_birth FROM patients WHERE id = $1',
    [visit.patient_id],
  )
  ...
  const mappingRow = await client.query(
    'SELECT external_patient_id FROM patient_ehr_ids WHERE patient_id = $1 AND ehr_connection_id = $2',
    [patient.id, connection.id],
  )
  let externalPatientId = mappingRow.rows[0]?.external_patient_id
  if (!externalPatientId) {
    externalPatientId = await driver.findOrCreatePatient(patient, connection.credentials)
    await client.query(
      ...
      [patient.id, connection.id, externalPatientId],
    )
  }

  const ehrEncounterId = await driver.pushEncounterNote({
    note: { final_note: finalNote },
    visit: { id: visit.id, visit_date: visit.visit_date },
    externalPatientId,
    externalProviderId: clinician.ehr_provider_id || null,
    credentials: connection.credentials,
  })

  return { ehrConnectionId: connection.id, ehrEncounterId }
}
...
const uploadToEHR = async (req, res) => {
  try {
    ...
    const out = await withTransaction(async (client) => {
      const noteRow = await client.query(
        'SELECT id, visit_id, status, ehr_uploaded_at, final_note FROM notes WHERE id = $1 FOR UPDATE',
        [id]
      )
      if (!noteRow.rows[0]) return { notFound: true }
      ...
      let ehrConnectionId = null
      let ehrEncounterId = null
      try {
        const pushResult = await pushNoteToEhrIfConfigured({ visit, finalNote: noteRow.rows[0].final_note, client })
        ehrConnectionId = pushResult.ehrConnectionId || null
        ehrEncounterId = pushResult.ehrEncounterId || null
      } catch (ehrErr) {
        return { ehrPushFailed: true, message: ehrErr.message }
      }
      ...
      const upd = await client.query(
        `UPDATE notes
            SET ehr_uploaded_at = NOW(),
                ehr_uploaded_by = $1,
                ehr_connection_id = COALESCE($2, ehr_connection_id),
                ehr_encounter_id  = COALESCE($3, ehr_encounter_id),
                updated_at      = NOW()
          WHERE id = $4 RETURNING *`,
        [scribe_id, ehrConnectionId, ehrEncounterId, id]
      )
    ...
    if (out.ehrPushFailed) {
      return res.status(502).json({ error: `Failed to push note to EHR: ${out.message}` })
    }
