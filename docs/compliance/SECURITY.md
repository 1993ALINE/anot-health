...
Run `npm audit` in `anot-backend-main` and `anot-frontend-main` before each release. Apply `npm audit fix` for patch-level issues with available fixes; document any remaining findings here.
