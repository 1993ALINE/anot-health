<#
================================================================================
 update-ops-policy-waf-logging.ps1  -  Add CloudWatch Logs *log-delivery*
                                       permissions to 'anot-ops-prod-policy'
================================================================================
 ...
 WHY THIS EXISTS:
   Enabling WAF logging (wafv2:PutLoggingConfiguration with a CloudWatch Logs
   destination) fails for the ops user with an AccessDenied on
   'logs:CreateLogDelivery'. WAF -> CloudWatch Logs uses the CloudWatch Logs
   "vended log delivery" mechanism, which requires the CALLER to hold the
   account-level log-delivery actions. This script grants them idempotently.
 ...
 WHAT THIS DOES:
   ...
   granting the log-delivery actions, then publishes the merged document as a
   NEW DEFAULT policy version via 'iam create-policy-version --set-as-default'.

 THE STATEMENT ADDED (Sid):
   WafLogDelivery   logs:CreateLogDelivery / logs:DeleteLogDelivery   (Resource *)

 WHY Resource = "*" (NOT aws-waf-logs-*):
   logs:CreateLogDelivery and logs:DeleteLogDelivery are ACCOUNT-LEVEL actions
   that do NOT support resource-level permissions; IAM requires Resource "*" for
   them. They therefore live in their own statement rather than being bolted onto
   the existing 'WafLogsCreateAndWrite' statement (which is correctly scoped to
   'aws-waf-logs-*' log groups for the resource-scopable actions like
   CreateLogGroup / PutLogEvents). Splitting keeps each statement least-privilege
 ...
 USAGE:
   powershell -File scripts/update-ops-policy-waf-logging.ps1 -DryRun   # rehearse
   powershell -File scripts/update-ops-policy-waf-logging.ps1           # apply (prompts)
   powershell -File scripts/update-ops-policy-waf-logging.ps1 -Force    # apply, no prompts
================================================================================
#>
...
trap {
    ...
    Write-Host '  UPDATE OPS POLICY (WAF LOGGING) FAILED' -ForegroundColor Red
    Write-Host ('=' * 78) -ForegroundColor Red
...
# ==============================================================================
# PHASE 2 - Merge in the log-delivery statement (idempotent, keyed by Sid)
# ==============================================================================
Write-Phase 'PHASE 2: Merge WAF log-delivery statement'
...
$wafStatements = @(
    [ordered]@{
        ...
        # Account-level actions: IAM does NOT support resource scoping here, so
        # Resource MUST be '*' for the grant to take effect.
        Resource = '*'
    }
)
...
if ($alreadyPresent.Count -gt 0) {
    Write-Step "Existing log-delivery statement found and will be refreshed: $($alreadyPresent -join ', ')"
} else {
    Write-Step "Adding new log-delivery statement: $($wafSids -join ', ')"
}
...
$mergedFile = Join-Path $ArtifactDir "anot-ops-policy-waf-logging-$Stamp.json"
...
if ($noChange) {
    Write-Ok 'Policy already contains the WAF log-delivery permissions (unchanged). Nothing to publish.'
    Write-Host ''
    return
}
...
Confirm-Step "Publish a NEW DEFAULT version of '$ManagedPolicyName' with the WAF log-delivery permissions?"
...
# ==============================================================================
# PHASE 4 - Verify the new default version contains the log-delivery actions
# ==============================================================================
...
Write-Host '  SUCCESS: WAF log-delivery permissions added to the ops policy' -ForegroundColor Green
Write-Host ('=' * 78) -ForegroundColor Green
...
Write-Host "  statement     : $($wafSids -join ', ')" -ForegroundColor Green
Write-Host "  actions       : logs:CreateLogDelivery, logs:DeleteLogDelivery (Resource *)" -ForegroundColor Green
Write-Host ''
Write-Host '  Saved document (audit):' -ForegroundColor Yellow
Write-Host "    $mergedFile" -ForegroundColor DarkGray
Write-Host ''
Write-Warn 'IAM changes can take a few seconds to propagate. If WAF logging still'
Write-Warn 'returns AccessDenied, wait ~10-30s and re-run enable-cloudfront-waf.ps1.'
Write-Host ''
Write-Ok 'The ops user can now run enable-cloudfront-waf.ps1 with logging enabled.'
