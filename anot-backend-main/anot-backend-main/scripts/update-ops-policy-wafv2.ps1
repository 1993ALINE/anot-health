<#
================================================================================
 update-ops-policy-wafv2.ps1  -  Add WAFv2 (CloudFront WAF) + WAF-logs
...
================================================================================
 ...
 WHAT THIS DOES:
   ...
   'anot-ops-prod-policy', merges in three statements (idempotently, keyed by
   Sid) granting the WAFv2 + CloudWatch-Logs permissions the ops user needs to
   create and manage a CloudFront WAF, then publishes the merged document as a
   NEW DEFAULT policy version via 'iam create-policy-version --set-as-default'.

 THE STATEMENTS ADDED (Sids):
   WafV2GlobalList        wafv2:ListWebACLs / ListIPSets / ListRuleGroups /
                          ListRules                                   (Resource *)
   WafV2ManageCloudFront  wafv2:GetWebACL / CreateWebACL / UpdateWebACL /
                          DeleteWebACL / Get+Put+DeleteLoggingConfiguration /
                          CreateIPSet / GetIPSet / UpdateIPSet /
                          CreateRuleGroup / GetRuleGroup / CreateRule /
                          TagResource / ListTagsForResource
                          (scoped to us-east-1 'global' WAFv2 resources)
   WafLogsCreateAndWrite  logs:CreateLogGroup / CreateLogStream / PutLogEvents /
                          PutRetentionPolicy   (scoped to aws-waf-logs-* groups)

 WHY us-east-1: WAFv2 for CloudFront uses scope=CLOUDFRONT and its control plane
   AND resources live ONLY in us-east-1 (ARN region 'us-east-1', partition path
   'global'). CloudFront WAF log groups must also be in us-east-1 and be named
   'aws-waf-logs-*'. So these ARNs are us-east-1, independent of the regional
   (ap-southeast-1) resources elsewhere in the policy.

 NOTE ON TWO REQUESTED ACTIONS: 'wafv2:ListRules' and 'wafv2:CreateRule' are WAF
   *Classic* actions; WAFv2 has no equivalents (rules live inside web ACLs and
   rule groups - manage them with Create/UpdateWebACL and CreateRuleGroup). They
   are included verbatim because they were requested; they are harmless but inert.
 ...
 SAFETY:
   * Idempotent: if the three Sids are already present and unchanged, no new
 ...
 USAGE:
   powershell -File scripts/update-ops-policy-wafv2.ps1 -DryRun   # rehearse
   powershell -File scripts/update-ops-policy-wafv2.ps1           # apply (prompts)
   powershell -File scripts/update-ops-policy-wafv2.ps1 -Force    # apply, no prompts
================================================================================
#>
...
# WAFv2 for CloudFront + its log groups are us-east-1 / 'global'-scoped.
$WafRegion       = 'us-east-1'
...
trap {
    ...
    Write-Host '  UPDATE OPS POLICY (WAFv2) FAILED' -ForegroundColor Red
    Write-Host ('=' * 78) -ForegroundColor Red
...
# ==============================================================================
# PHASE 2 - Merge in the WAFv2 + WAF-logs statements (idempotent, keyed by Sid)
# ==============================================================================
Write-Phase 'PHASE 2: Merge WAFv2 + WAF-logs statements'
...
$wafStatements = @(
    [ordered]@{
        ...
        Action   = @(
            ...
            'wafv2:ListRules'      # WAF Classic action; included as requested (inert in WAFv2)
        )
        Resource = '*'
    },
    [ordered]@{
        ...
        Action   = @(
            ...
            'wafv2:GetRuleGroup',
            'wafv2:CreateRule',    # WAF Classic action; included as requested (inert in WAFv2)
            'wafv2:TagResource',
...
$wafSids = @($wafStatements | ForEach-Object { $_.Sid })
...
$keptStatements = @($existingStatements | Where-Object {
    -not (($_.PSObject.Properties.Name -contains 'Sid') -and ($wafSids -contains $_.Sid))
})
$mergedStatements = @($keptStatements) + @($wafStatements)

$alreadyPresent = @($wafSids | Where-Object { $existingSids -contains $_ })
if ($alreadyPresent.Count -gt 0) {
    Write-Step "Existing WAF statements found and will be refreshed: $($alreadyPresent -join ', ')"
} else {
    Write-Step "Adding new WAF statements: $($wafSids -join ', ')"
}
...
# AWS CLI file:// parser rejects a UTF-8 BOM). Matches create-iam-ops-user.ps1.
$mergedFile = Join-Path $ArtifactDir "anot-ops-policy-wafv2-$Stamp.json"
...
if ($noChange) {
    Write-Ok 'Policy already contains these WAFv2 + WAF-logs permissions (unchanged). Nothing to publish.'
    Write-Host ''
    return
}
...
Confirm-Step "Publish a NEW DEFAULT version of '$ManagedPolicyName' with the WAFv2 permissions?"
...
# ==============================================================================
# PHASE 4 - Verify the new default version contains the WAFv2 actions
# ==============================================================================
...
$missing = @($wafSids | Where-Object { $verifySids -notcontains $_ })
if ($missing.Count -gt 0) {
    throw "Verification failed: new default version is missing statement(s): $($missing -join ', ')."
}
Write-Ok "New default version contains: $($wafSids -join ', ')"
...
Write-Host '  SUCCESS: WAFv2 permissions added to the ops policy' -ForegroundColor Green
Write-Host ('=' * 78) -ForegroundColor Green
...
Write-Host "  statements    : $($wafSids -join ', ')" -ForegroundColor Green
Write-Host "  waf resources : us-east-1 'global' (CLOUDFRONT scope)" -ForegroundColor Green
Write-Host "  waf log group : aws-waf-logs-* (us-east-1)" -ForegroundColor Green
Write-Host ''
...
Write-Warn 'which has also been updated with these statements. Re-running that script will'
Write-Warn 'keep the WAFv2 permissions; running an OLDER copy would drop them again.'
Write-Host ''
Write-Ok "The ops user can now run enable-cloudfront-waf.ps1."
