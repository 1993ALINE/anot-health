...
describe('calculateTranscribeTimeout', () => {
  ...
  test('caps at 15 minutes for very large files', () => {
    expect(calculateTranscribeTimeout(500)).toBe(900000)
  })

  test('respects DEEPGRAM_TIMEOUT_MS env as a floor', () => {
    process.env.DEEPGRAM_TIMEOUT_MS = '900000'
    expect(calculateTranscribeTimeout(1)).toBe(900000)
  })
})
...
describe('resolveTranscribeTimeoutMs', () => {
  ...
  test('scales up for large files', () => {
    ...
    expect(resolveTranscribeTimeoutMs(settings, 500 * 1024 * 1024)).toBe(900000)
  })
})
