...
const { checkRateLimit, checkCostLimit, trackCost } = require('../services/claudeCostTracking')
...
async function callAnthropicForNote(anthropic, settings, prompt, visitId) {
  ...
  // Rate/cost guardrails — previously only wired into a dead code path
  // (claudeService.js's generateMedicalNotes, never called), so CLAUDE_DAILY_LIMIT
  // and CLAUDE_RATE_LIMIT had no effect on real traffic. Checked here instead, on
  // the path that actually runs. A thrown error here propagates through
  // generateAINote's catch block below and returns null, same as any other
  // Anthropic API failure — no new error-handling shape needed.
  checkRateLimit()
  checkCostLimit()

  // Deliberately NOT awaited at either call site below. The note has already been
  // generated by the time this runs — a slow or transiently-unreachable RDS
  // connection (the audit-log write) must never add latency to, or risk failing,
  // delivery of a clinical note that already succeeded. The in-memory daily-cost
  // counters (used by checkCostLimit above) still update synchronously inside
  // trackCost before its DB write — only the DB persistence is deferred.
  function recordUsage(response, modelUsed) {
    ...
    trackCost(
      ...
      console.warn(`[aiPipeline] trackCost failed for visit ${visitId ?? 'unknown'} (note was already generated, unaffected):`, err.message)
    })
  }
  ...
  try {
    ...
    if (err?.status === 404 || String(err?.message || '').toLowerCase().includes('model')) {
      ...
      recordUsage(fallbackResponse, fallbackModel)
...
async function generateAINote(transcriptions, patientInfo, templateSections, visitId, customInstructions) {
  ...
  try {
    ...
    const response = await callAnthropicForNote(loaded.client, loaded.settings, prompt, visitId)
